...
If IsWorkstationLocked() = 0 Then
   MessageToAdmin("W;" & @ScriptName & ";" & @YEAR & "-" & @MON & "-" & @MDAY & ";" & @HOUR & ":" & @MIN & ":" & @SEC & ";" & @UserName & ";" & @ComputerName & ";" & $CliParams[$CLI_COMPANYCODE] & ";" & $CliParams[$CLI_SYSTEM],"Workstation is locked. Connect to the HADES workstation and login/unlock user profile.",$SYS_ADMINS, Null)
   Exit(99)
EndIf
#EndRegion
...
For $arg = 1 To $CmdLine[0]
   ...
   Case "-CC"
	  If $CmdLine[0] > $arg Then
		 $CliParams[$CLI_COMPANYCODE] = StringUpper($CmdLine[$arg + 1])
		 $ActualParamsPassed = BitShift($ActualParamsPassed,-1)
	  EndIf
...
;There are 5 mandatory parameters at this point
If $ActualParamsPassed <> BitShift(1,$MANDATORY_PARAMS) Then
   ConsoleWrite("Usage: " & @ScriptName & " -s | --system $SAP_SYSTEM_NAME -c | --client $SAP_CLIENT_NAME -cc | --companycode $COMPANY_CODE -oawd $OAWD_FOLDER -src | --source $SHAREPOINT_URL_OR_NETWORK_SHARE_PATH [-as | --archivesharepoint] [-nal | --noarchlocal] [-nosub | --nosubdir]" & @CRLF)
   Exit(1)
Else
   ConsoleWrite("--------------------------------------------------" & @CRLF)
...
DirCreate($AUTO_FOLDER_ROOT & "\" & $CliParams[$CLI_COMPANYCODE] & $PROJECT)
DirCreate($AUTO_FOLDER_ROOT & "\" & $CliParams[$CLI_COMPANYCODE] & $PROJECT & "\" & $CliParams[$CLI_COMPANYCODE] & $PROJECT & "_" & $AUTO_FOLDER_SOURCE)
DirCreate($AUTO_FOLDER_ROOT & "\" & $CliParams[$CLI_COMPANYCODE] & $PROJECT & "\" & $CliParams[$CLI_COMPANYCODE] & $PROJECT & "_" & $AUTO_FOLDER_ARCHIVE)
$LogFile = FileOpen($AUTO_FOLDER_ROOT & "\" & $CliParams[$CLI_COMPANYCODE] & $PROJECT & "\" & $RunTimeStamp & ".log.txt", $FO_APPEND)
$LogFilePath = $AUTO_FOLDER_ROOT & "\" & $CliParams[$CLI_COMPANYCODE] & $PROJECT & "\" & $RunTimeStamp & ".log.txt"
#EndRegion
...
If StringRegExp($CliParams[$CLI_SOURCE], "(\\{2}[a-zA-Z0-9\.]*){1}\\.*") Or StringRegExp($CliParams[$CLI_SOURCE], "^[a-zA-Z]:\\[\\\S|*\S]?.*$", $STR_REGEXPMATCH) Then
   If Not FileExists($CliParams[$CLI_SOURCE]) Then
	  ...
	  MessageToAdmin("E;" & @ScriptName & ";" & $CliParams[$CLI_COMPANYCODE] & ";" & $CliParams[$CLI_SYSTEM],"Exception occured. See the attached log file", $SYS_ADMINS, $LogFilePath)
...
ElseIf StringRegExp($CliParams[$CLI_SOURCE], "(https?:\/\/(?:www\.|(?!www))[a-zA-Z0-9][a-zA-Z0-9-]+[a-zA-Z0-9]\.[^\s]{2,}|www\.[a-zA-Z0-9][a-zA-Z0-9-]+[a-zA-Z0-9]\.[^\s]{2,}|https?:\/\/(?:www\.|(?!www))[a-zA-Z0-9]+\.[^\s]{2,}|www\.[a-zA-Z0-9]+\.[^\s]{2,})", $STR_REGEXPMATCH) Then
   $FileCount = SPFolderGetFileCount($Http, $CliParams[$CLI_SOURCE], $SpAccessToken, $CliParams[$CLI_SUBDIRS], $CliParams[$CLI_COMPANYCODE], $FilesToProcess)
   If @error Then
	  LogEvent($LogFile, "Failed", True)
	  MessageToAdmin("E;" & @ScriptName & ";" & $CliParams[$CLI_COMPANYCODE] & ";" & $CliParams[$CLI_SYSTEM],"Exception occured. See the attached log file", $SYS_ADMINS, $LogFilePath)
...
If $FileCount = 0 Then
   ...
   MessageToAdmin("I;" & @ScriptName & ";" & $CliParams[$CLI_COMPANYCODE] & ";" & $CliParams[$CLI_SYSTEM],"No files for processing found in the location: " & $CliParams[$CLI_SOURCE], $SYS_ADMINS, $LogFilePath)
   Exit(2) ; Nothing to process
EndIf

LogEvent($LogFile, "Data source " & $DataSource, False)
LogEvent($LogFile, $FileCount & " file(s) discovered", False)
#EndRegion


#Region check for residual file(s)
Local $ResidualFiles = DirGetSize($AUTO_FOLDER_ROOT & "\" & $CliParams[$CLI_COMPANYCODE] & $PROJECT & "\" & $CliParams[$CLI_COMPANYCODE] & $PROJECT & "_" & $AUTO_FOLDER_SOURCE, $DIR_EXTENDED)
If $ResidualFiles[1] > 0 Then
    LogEvent($LogFile, "Residual file (" & $ResidualFiles[1] & ") found in the local source location " & $AUTO_FOLDER_ROOT & "\" & $CliParams[$CLI_COMPANYCODE] & $PROJECT & "\" & $CliParams[$CLI_COMPANYCODE] & $PROJECT & "_" & $AUTO_FOLDER_SOURCE, True)
    MessageToAdmin("E;" & @ScriptName & ";" & $CliParams[$CLI_COMPANYCODE] & ";" & $CliParams[$CLI_SYSTEM],"Residual file(s) found in the local source. See the attached log file", $SYS_ADMINS, $LogFilePath)
	Exit(98)
EndIf
#EndRegion
...
If StringRegExp(StringUpper($SAP[$SAP_SES].Info.Transaction), "S000") Then
   ...
   MessageToAdmin("E;" & @ScriptName & ";" & $CliParams[$CLI_COMPANYCODE] & ";" & $CliParams[$CLI_SYSTEM], "Failed to login: " & $Net.UserName & " into " & $CliParams[$CLI_SYSTEM] & " " & $CliParams[$CLI_CLIENT], $SYS_ADMINS, $LogFilePath)
EndIf
...
Local $PathSrc = $AUTO_FOLDER_ROOT & "\" & $CliParams[$CLI_COMPANYCODE] & $PROJECT & "\" & $CliParams[$CLI_COMPANYCODE] & $PROJECT & "_" & $AUTO_FOLDER_SOURCE
Local $PathArch = $AUTO_FOLDER_ROOT & "\" & $CliParams[$CLI_COMPANYCODE] & $PROJECT & "\" & $CliParams[$CLI_COMPANYCODE] & $PROJECT & "_" & $AUTO_FOLDER_ARCHIVE
Local $ExplorerTitle = $CliParams[$CLI_COMPANYCODE] & $PROJECT & "_" & $AUTO_FOLDER_SOURCE
...
Else
   ...
   If Not OpenWindowsExplorer($PathSrc) Then
	  ...
	  MessageToAdmin("E;" & @ScriptName & ";" & $CliParams[$CLI_COMPANYCODE] & ";" & $CliParams[$CLI_SYSTEM],"", $SYS_ADMINS, $LogFilePath)
	  Exit(120)
   EndIf
...
Local $InvalidFiles = 0
Local $ValidFiles = 0
Local $DropResult
Local $Timer = TimerInit()
Local $File
Local $ExplorerWindowTitle = $CliParams[$CLI_COMPANYCODE] & $PROJECT & "_" & $AUTO_FOLDER_SOURCE
Local $LocalArchiveFolder = $AUTO_FOLDER_ROOT & "\" & $CliParams[$CLI_COMPANYCODE] & $PROJECT & "\" & $CliParams[$CLI_COMPANYCODE] & $PROJECT & "_" & $AUTO_FOLDER_ARCHIVE & "\" & $RunTimeStamp
Local $LocalSourceFolder = $AUTO_FOLDER_ROOT & "\" & $CliParams[$CLI_COMPANYCODE] & $PROJECT & "\" & $CliParams[$CLI_COMPANYCODE] & $PROJECT & "_" & $AUTO_FOLDER_SOURCE
...
For $File In $FilesToProcess.Items
   ...
   ; **********************************************************************
   $ResidualFiles = DirGetSize($AUTO_FOLDER_ROOT & "\" & $CliParams[$CLI_COMPANYCODE] & $PROJECT & "\" & $CliParams[$CLI_COMPANYCODE] & $PROJECT & "_" & $AUTO_FOLDER_SOURCE, $DIR_EXTENDED)
   If $ResidualFiles[1] > 0 Then
	  LogEvent($LogFile, "Residual file (" & $ResidualFiles[1] & ") found in the local source location " & $AUTO_FOLDER_ROOT & "\" & $CliParams[$CLI_COMPANYCODE] & $PROJECT & "\" & $CliParams[$CLI_COMPANYCODE] & $PROJECT & "_" & $AUTO_FOLDER_SOURCE, True)
	  MessageToAdmin("E;" & @ScriptName & ";" & $CliParams[$CLI_COMPANYCODE] & ";" & $CliParams[$CLI_SYSTEM],"Residual file(s) found in the local source. See the attached log file", $SYS_ADMINS, $LogFilePath)
   ...
   ; ******************************************************************
   ; STATE_INITIAL -> STATE_INVALID || STATE_OK
   ;*******************************************************************
   ...
   If $File[$FILE_STATUS] = $STATE_DRAGDROPREADY Then
	  ...
	  Else
		 ...
		 MessageToAdmin("E;" & @ScriptName & ";" & $CliParams[$CLI_COMPANYCODE] & ";" & $CliParams[$CLI_SYSTEM], "Drag and drop failed with the following file: " & $File[$FILE_NAME] & " (" & $File[$FILE_PARENTFOLDER] & ")" & @CRLF _
   ...
   If $File[$FILE_STATUS] = $STATE_ARCHIVEDSP Or $File[$FILE_STATUS] = $STATE_ARCHIVEDLOCAL Or $File[$FILE_STATUS] = $STATE_DRAGDROPPED Then
      ...
      Switch $DataSource
		 ...
		 Case $SOURCE_SHAREPOINT
			...
			If SPDeleteFile($Http, $File[$FILE_PATH], $SpAccessToken) Then
			   FileDelete($LocalSourceFolder); Also safely delete local copy in the source folder
			   $File[$FILE_STATUS] = $STATE_DELETEDREMOTE
			   LogEvent($LogFile, $File[$FILE_NAME] & " (" & $File[$FILE_PARENTFOLDER] & ") (" & $FileState[$File[$FILE_STATUS]] & ")", False)
			EndIf
   ...
   ; ******************************************************************
   If Not $File[$FILE_STATUS] = $STATE_DELETEDREMOTE Or Not $File[$FILE_STATUS] = $STATE_INVALID Then
	  ConsoleWrite("File status is something else then deleted or invalid" & @CRLF)
	  LogEvent($LogFile,"Fail", True) ; Just close the log file
	  MessageToAdmin("E;" & @ScriptName & ";" & $CliParams[$CLI_COMPANYCODE] & ";" & $CliParams[$CLI_SYSTEM], "File " & $File[$FILE_NAME] & " (" & $File[$FILE_PARENTFOLDER] & ") did not reach the final state. Most recent state is: " & @CRLF _
...
Wdapp($CliParams[$CLI_COMPANYCODE] & $PROJECT)
LogEvent($LogFile,"DONE", True)

If $InvalidFiles <> 0 Then
   MessageToAdmin("W;" & @ScriptName & ";" & $CliParams[$CLI_COMPANYCODE] & ";" & $CliParams[$CLI_SYSTEM],"Processing of " &  $FilesToProcess.Count & " files took " & Floor(TimerDiff($Timer) / 1000) & " seconds. Average processing time per file " & Round(Floor(TimerDiff($Timer) / 1000) / $FilesToProcess.Count, 2) & " seconds. " & $InvalidFiles & " invalid file(s). See the attached log" ,$SYS_ADMINS, $LogFilePath)
Else
   MessageToAdmin("I;" & @ScriptName & ";" & $CliParams[$CLI_COMPANYCODE] & ";" & $CliParams[$CLI_SYSTEM],"Processing of " &  $FilesToProcess.Count & " files took " & Floor(TimerDiff($Timer) / 1000) & " seconds. Average processing time per file " & Round(Floor(TimerDiff($Timer) / 1000) / $FilesToProcess.Count, 2) & " seconds. " & $InvalidFiles & " invalid file(s). See the attached log" ,$SYS_ADMINS, $LogFilePath)
EndIf
...
Func NSFolderGetFileCount($_sNetworkPath, $_processSubdirs, ByRef $_dFilesToProcess)
   ...
   ElseIf $_processSubdirs And $Folder.Subfolders.Count >= 1 Then
	  For $Subfolder in $Folder.Subfolders
		 If StringRegExp($Subfolder.Name,$CliParams[$CLI_COMPANYCODE]) Then
...
Func SPFolderGetFileCount(ByRef $_oHTTP, $_sSiteUrl, $_sSecurityToken, $_bSubdirs, $_sSubdirPrefix, ByRef $_dFilesToProcess)
   ...
   Local $__Xml = ObjCreate("MSXML2.DOMDocument")
   Local $__XmlFiles
   Local $__XmlFolders
   Local $__XmlFile
   Local $__XmlFolder
   ...
   $__BaseApiUrl = $_sSiteUrl & "_api/"
   $_sSiteUrl = $_sSiteUrl & "_api/web/GetFolderByServerRelativeUrl('" & $__SharepointDir & "')"

   ;******************************
   ;Do not process subdirs
   ;******************************
   If Not $_bSubdirs Then

	  With $_oHTTP
		 .open("GET", $__BaseApiUrl & "web/GetFolderByServerRelativeUrl('" & $__SharepointDir & "')/Files", False)
	  ...
	  If $_oHTTP.Status <> 200 Then
		 LogEvent($LogFile, "Error getting list of files (" & $__SharepointDir & ") HTTP Status: " & $_oHTTP.responseText & ". Stopping execution", False)
		 SetError(305) ; Can't get list of file from sharepoint
		 Return 0
	  EndIf

	  $__Xml.LoadXML($_oHTTP.ResponseText)

	  If $__Xml.SelectNodes("//entry").length = 0 Then
		 Return 0 ; No files found
	  EndIf

	  ; Files found
      $__XmlFiles = $__Xml.SelectNodes("//entry")

	  For $__XmlFile In $__XmlFiles
		 ; parent folder computation no needed here. simply take $__SharepointDir
		 ;$FILE_PARENTFOLDER = 0, $FILE_PATH, $FILE_NAME, $FILE_SIZE, $FILE_SOURCETYPE, $FILE_STATUS
		 Local $__filePropertiesArray[6] = [$__SharepointDir, $__BaseApiUrl & $__XmlFile.selectSingleNode("link[@rel='edit']").attributes.getNamedItem("href").text, $__XmlFile.selectSingleNode("content/m:properties/d:Name").text, $__XmlFile.selectSingleNode("content/m:properties/d:Length").text, 1, 0]
		 $_dFilesToProcess.Add($__XmlFile.selectSingleNode("content/m:properties/d:Name").text, $__filePropertiesArray)
	  Next
   ...
   ;Replace with company code
	  ...
	  If $_oHTTP.Status <> 200 Then
		 LogEvent($LogFile, "Error getting list of subfolders (" & $__SharepointDir & ") HTTP Status: " & $_oHTTP.responseText & ". Stopping execution", False)
		 SetError(306) ; Can't get list of folders
		 Return 0
	  EndIf
      ...
      $__XmlFolders = $__Xml.SelectNodes("//entry") ; Individual folders

	  For $__XmlFolder In $__XmlFolders

		 With $_oHTTP
			.open("GET", $__BaseApiUrl & $__XmlFolder.selectSingleNode("link[@title='Files']").attributes.getNamedItem("href").text, False)
			.setRequestHeader("Authorization", "Bearer " & $_sSecurityToken)
			.setRequestHeader("Accept", "application/atom+xml")
	        .send()
		 EndWith
		 ...
		 If $_oHTTP.Status <> 200 Then
			LogEvent($LogFile, "Error getting list of files (" & $__XmlFolder.selectSingleNode("content/m:properties/d:Name").text & ") HTTP Status: " & $_oHTTP.responseText & ". Stopping execution", False)
			SetError(305)
			Return 0
		 EndIf


		 $__Xml.LoadXML($_oHTTP.ResponseText) ; All files
		 $__XmlFiles = $__Xml.SelectNodes("//entry") ; Individual files
		 LogEvent($LogFile, "Number of files in folder " & $__XmlFolder.selectSingleNode("content/m:properties/d:Name").text & " is " & $__Xml.SelectNodes("//entry").length, False)

		 For $__XmlFile In $__XmlFiles

			;$FILE_PARENTFOLDER = 0, $FILE_PATH, $FILE_NAME, $FILE_SIZE, $FILE_SOURCETYPE, $FILE_STATUS
			Local $__filePropertiesArray[6] = [$__XmlFolder.selectSingleNode("content/m:properties/d:Name").text, $__BaseApiUrl & $__XmlFile.selectSingleNode("link[@rel='edit']").attributes.getNamedItem("href").text, $__XmlFile.selectSingleNode("content/m:properties/d:Name").text, $__XmlFile.selectSingleNode("content/m:properties/d:Length").text, 1, 0]
			$_dFilesToProcess.Add($__XmlFolder.selectSingleNode("content/m:properties/d:Name").text & "_" & $__XmlFile.selectSingleNode("content/m:properties/d:Name").text , $__filePropertiesArray)
...
Func SPFolderGetFileCountA(ByRef $_oHTTP, $_sSiteUrl, $_sSecurityToken, $_processSubdirs)
    ...
    If Not $_processSubdirs Then
	  ...
	  $__FileCount = $__XmlFiles.selectNodes("//entry").length
   ...
   Else
     ...
     For $__Node in $__XmlFolders.selectNodes("//m:properties/d:Name")
		...
		If StringRegExp($__Node.text,$CliParams[$CLI_COMPANYCODE]) Then
		   $__SubfoldersCount = $__SubfoldersCount + 1
...
Func SPDownloadFile(ByRef $_oHTTP, $_sFileUrl, $_sDestinationPath, $_sSecurityToken)
   Local $__oStream = ObjCreate("ADODB.Stream")

   With $_oHTTP
	  .open("GET", $_sFileUrl & "/$value", False)
	  .setRequestHeader("Authorization", "Bearer " & $_sSecurityToken)
	  .setRequestHeader("Accept", "application/atom+xml")
	  .send()
   EndWith
   ...
   If $_oHTTP.status <> 200 Then
	  LogEvent($LogFile, "Error downloading file. HTTP Status: " & $_oHTTP.responseText & ". Stopping execution", False)
	  Return False
   EndIf
   ...
   With $__oStream
	  .Open()
	  .Type = 1
	  .Write($_oHTTP.responseBody)
	  .SaveToFile($_sDestinationPath)
	  .Close()
   EndWith
...
Func SPDeleteFile(ByRef $_oHTTP, $_sFileUrl, $_sSecurityToken)

   With $_oHTTP
	  .open("POST", $_sFileUrl, False)
	  .setRequestHeader("Authorization", "Bearer " & $_sSecurityToken)
	  .setRequestHeader("Accept", "application/atom+xml")
   ...
   If $_oHTTP.status <> 200 Then
	  LogEvent($LogFile, "Error deleting file. HTTP Status: " & $_oHTTP.responseText & ". Stopping execution", False)
	  Return False
   Else
	  Return True
   EndIf
...
Func ErrFunc($oError)
   ...
   MessageToAdmin("W;" & @ScriptName & ";" & @YEAR & "-" & @MON & "-" & @MDAY & ";" & @HOUR & ":" & @MIN & ":" & @SEC & ";" & @UserName & ";" & @ComputerName & ";" & $CliParams[$CLI_COMPANYCODE] & ";" & $CliParams[$CLI_SYSTEM],"We intercepted a COM Error ! Number: 0x" & Hex($oError.number, 8) & " Description: " & $oError.windescription & " At line: " & $oError.scriptline, $SYS_ADMINS, Null)
   Exit(999)
